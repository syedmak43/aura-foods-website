...
from database import get_testimonials, get_bundles, get_blog_posts, get_blog_post, get_why_items, get_messages
...
@app.post("/contact")
async def contact_submit(request: Request, name: str = Form(...), email: str = Form(...), phone: str = Form(""), message: str = Form(...)):
    from database import get_db
    conn = get_db()
    conn.execute("CREATE TABLE IF NOT EXISTS contact_messages (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, email TEXT, phone TEXT, message TEXT, created_at TEXT DEFAULT (datetime('now')))")
    conn.execute("INSERT INTO contact_messages (name,email,phone,message) VALUES (?,?,?,?)", (name,email,phone,message))
    conn.commit(); conn.close()
    meta = seo_meta("Contact Aura Foods", "Reach out for orders, wholesale inquiries, or any questions.", "https://aurafoods.pk/contact")
    return templates.TemplateResponse("contact.html", {"request": request, "meta": meta, "settings": {k: get_setting(k) for k in ["phone","email","address","whatsapp"]}, "success": "Thank you! We'll get back to you within 24 hours."})
